...
const DesTasksItem = ({ agregarPedido, item, handleSubmit }) => {
  const AlPedido= ()=>{
    if(item.Nota !== 'AGOTADO'){
      agregarPedido({cod: item.cod, Descripcion: item.Descripcion, UnidadOpaquete: item.UnidadOpaquete, EsUnidadOpaquete: item.EsUnidadOpaquete, SubCategoria: item.SubCategoria, PVenta: item.PVenta, Cantidad: item.EsUnidadOpaquete, Costo: item.PCosto, Nota: item.Nota});
      handleSubmit('')
    }
  }
