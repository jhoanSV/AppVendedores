...
import { StyleSheet, TextInput, Text, View, Image, ImageBackground, Modal, TouchableOpacity , Dimensions} from "react-native";
...
const LogIn = () => {
  ...
  const handleSubmit = async() => {
    ...
    // Crea un timeout que rechaza después de X milisegundos
    const timeout = new Promise((_, reject) => 
        setTimeout(() => reject(new Error('timeout')), 10000) // 10 segundos
    );
    try {
      const userData = await checkLogindDataApi(tasks)
      console.log('userData',userData)
      if(!userData){ 
          return;
      }
      if(userData.Cod){
        setUser(userData.Cod)
      ...
      if (error.message === 'timeout') {
        setWarningText('Error de conección')
        setVisible(true)
        setCargando(false)
        setTimeout(() => {  
          setVisible(false)
  ...
  const limpiar = () => setTasks({...tasks, ["User"]: "", ["Contraseña"]: ""});
